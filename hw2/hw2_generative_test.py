# generative test
# type "python hw2_generative_test.py {X_test} {hw2_generative.csv}" to execute
...
def testing(x_test, w):
    ...
    # predict
    prob = sigmoid(np.dot(x_test, w)) # probability of being class 0
    mask = (prob >= 0.5)
    y = np.ones(len(x_test))
    y[mask] = 0
...
def main(script, X_test, output):
    ...
    # feature scaling
    x_mean = np.load('./hw2_generative_model/x_mean.npy')
    x_std = np.load('./hw2_generative_model/x_std.npy')
    x_test = feature_scaling(x_test, x_mean, x_std)

    w = np.load('./hw2_generative_model/model.npy')
